...
define([
	...
	WysiwygWidget.Widget.prototype = {
		...
		initialize: function(formEl, widgetEl, widgetOptionsEl, optionsSourceUrl, widgetTargetId) {
			...
			if (typeof(tinyMCE) != "undefined" && window.tinymce.activeEditor) {
				this.bMark = window.tinymce.activeEditor.selection.getBookmark();
			}
						...
						onComplete: function(transport) {
							try {
								...
								if (typeof(tinyMCE) != "undefined" && window.tinymce.activeEditor) {
									...
									if (this.bMark) {
										window.tinymce.activeEditor.selection.moveToBookmark(this.bMark);
									}
								}
		...
		wysiwygExists: function() {
			return (typeof tinyMCE != 'undefined') && window.tinymce.get(this.widgetTargetId);
		},
		...
		getWysiwygNode: function() {
			return window.tinymce.activeEditor.selection.getNode();
		}
	}
